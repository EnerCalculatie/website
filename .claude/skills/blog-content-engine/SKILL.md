...
- **`checkSpellingGrammar` (`scripts/lib/spelling-check.mjs`):** spelling/grammatica via de publieke LanguageTool-API (nl, geen key nodig). Alleen TYPOS/GRAMMAR/CASING/PUNCTUATION zijn blokkerend, STYLE-suggesties genegeerd (te subjectief). **Best-effort:** is de publieke API onbereikbaar/rate-limited, wordt dat gelogd en de check overgeslagen in plaats van de hele pipeline te blokkeren op een derde-partij-storing.
