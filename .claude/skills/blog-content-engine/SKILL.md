...
- **`checkSpellingGrammar` (`scripts/lib/spelling-check.mjs`):** spelling/grammatica via de publieke LanguageTool-API (nl, geen key nodig). Alleen TYPOS/GRAMMAR/CASING/PUNCTUATION zijn blokkerend, STYLE-suggesties genegeerd (te subjectief). **Best-effort:** is de publieke API onbereikbaar/rate-limited, wordt dat gelogd en de check overgeslagen in plaats van de hele pipeline te blokkeren op een derde-partij-storing. **`ai-context/known-terms.json`** is de allowlist voor vakjargon/afkortingen die LanguageTool niet kent (MIA, VAMIL, OCPP...) — zonder filter faalt elk artikel over subsidies/techniek permanent op een terecht-klinkende maar valse spelfout. Nieuwe onbekende term steeds gevlagd? Toevoegen aan die JSON, geen codewijziging nodig.
